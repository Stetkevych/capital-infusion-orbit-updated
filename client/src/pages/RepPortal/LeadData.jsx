...
export default function LeadData() {
  ...
  const loadMetrics = async (d) => { setLoading(true); setError(''); try { const res = await fetch(`${API}/calendly/metrics?days=${d}`, { headers }); if (!res.ok) throw new Error('Failed to load'); setMetrics(await res.json()); } catch (e) { setError(e.message); } setLoading(false); };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs">Calendly + Zoho CRM · Waymo channel</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => loadMetrics(days)} className="p-2 bg-gray-100 hover:bg-gray-200 rounded-lg"><RefreshCw size={13} className="text-gray-500" /></button>
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-4">
              <p className="text-gray-400 text-xs font-medium mb-1">Total Meetings</p>
              <p className="text-gray-900 text-xl font-bold">{metrics.totals.total_meetings}</p>
            </div>
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-4">
              <p className="text-gray-400 text-xs font-medium mb-1">No Shows</p>
              <p className="text-red-500 text-xl font-bold">{metrics.totals.no_shows}</p>
            </div>
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-4">
              <p className="text-gray-400 text-xs font-medium mb-1">Net Meetings</p>
              <p className="text-gray-900 text-xl font-bold">{metrics.totals.net_meetings}</p>
            </div>
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-4">
              <p className="text-gray-400 text-xs font-medium mb-1">Show-Up Rate</p>
              <p className="text-green-600 text-xl font-bold">{metrics.totals.show_up_rate}%</p>
            </div>
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-4">
              <p className="text-gray-400 text-xs font-medium mb-1">Lead→App*</p>
              <p className="text-blue-600 text-xl font-bold">{metrics.totals.lead_to_app_rate}%</p>
              <p className="text-gray-300 text-xs">{metrics.totals.apps} apps</p>
            </div>
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-4">
              <p className="text-gray-400 text-xs font-medium mb-1">Funding Rate*</p>
              <p className="text-purple-600 text-xl font-bold">{metrics.totals.funding_rate}%</p>
              <p className="text-gray-300 text-xs">{metrics.totals.funded} funded</p>
            </div>
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-4">
              <p className="text-gray-400 text-xs font-medium mb-1">Avg Deal / Pts</p>
              <p className="text-gray-900 text-xl font-bold">{money(metrics.totals.avg_deal_size)}</p>
              <p className="text-gray-300 text-xs">{metrics.totals.avg_pts} pts/deal</p>
            </div>
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-4">
              <p className="text-gray-400 text-xs font-medium mb-1">Cost</p>
              <p className="text-gray-900 text-xl font-bold">{money(metrics.totals.cost)}</p>
              <p className="text-gray-300 text-xs">@ $200/mtg</p>
            </div>
          </div>

          <p className="text-gray-400 text-xs italic">* Lead→App and Funding Rate are calculated with no-shows factored out. Denominator = net meetings (those who showed up).</p>

          <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <h3 className="text-gray-900 font-semibold text-sm">Rep Breakdown</h3>
              <p className="text-gray-400 text-xs mt-0.5">Click headers to sort · Last {days} days</p>
            </div>
                ...
                <tbody className="divide-y divide-gray-50">
                  {sortedReps.map(([name, s]) => (
                    <tr key={name} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3"><span className={`px-2 py-0.5 text-xs rounded-full font-medium ${s.show_up_rate >= 80 ? 'bg-green-50 text-green-600' : s.show_up_rate >= 60 ? 'bg-amber-50 text-amber-600' : s.show_up_rate != null ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-400'}`}>{pct(s.show_up_rate)}</span></td>
                      <td className="px-4 py-3 text-gray-700">{s.apps || 0}</td>
                      <td className="px-4 py-3"><span className={`px-2 py-0.5 text-xs rounded-full font-medium ${s.lead_to_app_rate >= 30 ? 'bg-green-50 text-green-600' : s.lead_to_app_rate >= 15 ? 'bg-amber-50 text-amber-600' : s.lead_to_app_rate != null ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-400'}`}>{pct(s.lead_to_app_rate)}</span></td>
